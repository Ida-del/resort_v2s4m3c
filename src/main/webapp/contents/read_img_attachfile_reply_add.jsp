...
<script type="text/javascript">
  $(function() { // 자동 실행
    ...
    $('#content',frm_reply).on('click', check_login);   // 댓글 등록시 로그인 여부 체크
    $('#btn_create',frm_reply).on('click', reply_create);   // 댓글 등록

    //    list_by_contentsno_join();   // 댓글목록 호출
    list_by_contentsno_join_add();   // 댓글 목록 + 페이징지원 호출
  ...
  function reply_create() {
    var frm_reply = $('#frm_reply'); // frm serialize 사용(중요)
    
    if(check_login() != false) {  // 로그인 할 때만 처리
      var params = frm_reply.serialize();
      // alert(params);  // params 확인용=
      // return;

      // 자바스크립트는 영숫자, 한글, 공백, 특수문자 등을 글자수 1로 인식, 
      // 오랔클은 한글 1자가 3바이트로 할당됨으로 300자로 제한
      ...
      $.ajax({
        ...
        success: function(rdata) { // 서버로부터 성공적으로 응답이 온경우
          ...
          if (rdata.cnt > 0) {
           ...
           // list_by_contentsno_join();   // (중요) 댓글들 출력
           
           $('#reply_list').html("");   //  div 태그 초기화 기능
           $('#reply_list').attr("data-replyPage",1);  // 등록시 댓글이 새로 통합됨으로 1로 초기화시키는 코드.
            list_by_contentsno_join_add();  // paging 댓글
          } else {
    ...
    $.ajax({
      ...
      success: function(rdata) { // 서버로부터 성공적으로 응답이 온경우
        ...
        for (i=0; i < rdata.list.length; i++) {
          ...
          msg += "  " + row.rdate;
          if ('${sessionScope.memberno}' == row.memberno) { // 글쓴이 일치여부 확인, 글쓴이만! 삭제시키는 코드
        ...
        // alert(msg);
        $('#reply_list').append(msg);  // 아래 div 태그로 출력~
      },
      // Ajax 통신 에러, 응답 코드가 200이 아닌경우, dataType이 다른경우 
      error: function(request, status, error) { // callback 함수
        var msg = 'ERROR request.status: '+request.status + '/ ' + error;
  ...
  function reply_delete(replyno) {
    ...
    $('#modal_panel_delete').modal();               // 삭제폼(패스워드 체크) 다이얼로그 출력
  }
      ...
      success: function(rdata) { // 서버로부터 성공적으로 응답이 온경우
        ...
        if (rdata.passwd_cnt ==1) { // 패스워드 일치
          if (rdata.delete_cnt == 1) { // 삭제 성공
            ...
            $('#btn_frm_reply_delete_close').trigger("click"); // 삭제폼 닫기, click 발생 시키는 코드 
            
            $('#' + replyno).remove(); // 태그 삭제(중요 코드)
              
            return; // 함수 실행 종료
          } else {  // 패스워드 일치 but 삭제 실패
  ...
  function list_by_contentsno_join_add() {
    ...
    $.ajax({
      ...
      data: params,        // 서버로 전달하는 데이터
      success: function(rdata) { // 서버로부터 성공적으로 응답이 온경우
        $("#reply_list").attr("data-replyPage", replyPage+1);  // (중요)개발자정의 속성 쓰기, 페이지수 증가, 태그에서 증가시킴
        // alert(rdata);
        var msg = '';
        ...
        // alert(msg);
        $('#reply_list').append(msg);   // .append: reply_list Body 마지막 부분에 추가됨.
      },
      // Ajax 통신 에러, 응답 코드가 200이 아닌경우, dataType이 다른경우 
      error: function(request, status, error) { // callback 함수
        var msg = 'ERROR request.status: '+request.status + '/ ' + error;
